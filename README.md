...
> **버전**: 1.11.0  
> **대상**: Windows 10/11 + PowerShell + Claude Code v2.0+

## 변경 이력 (Changelog)

### 1.11.0 — Progressive Disclosure + 아키텍처 패턴 명시 + 검증 도구

**revfactory/harness 검토 후 부분 차용** (대체가 아닌 보완)

- **Progressive Disclosure 도입**
  - `implement-task/SKILL.md` 522줄 → 본문 ~290줄 + reference/ 4개 파일
    - `reference/halt-conditions.md` — 중단 조건 + 보고 양식
    - `reference/recovery.md` — 복구 메커니즘
    - `reference/antipatterns.md` — 안티패턴 표 (22개)
    - `reference/phase-f-detail.md` — Phase F 상세 (F-1~F-7)
  - `plan-feature/SKILL.md` 409줄 → 본문 ~220줄 + reference/ 3개 파일
    - `reference/decision-points.md` — 11개 카테고리 + 기록 형식
    - `reference/edge-cases.md` — Edge Cases 10개 + Halt Forecast
    - `reference/plan-template.md` — plan.md 템플릿
  - 본문은 핵심 흐름만, 상세는 필요 시 Read

- **아키텍처 패턴 명시**
  - README 개요에 "Pipeline + Producer-Reviewer" 패턴 명시
  - 다른 4개 패턴(Fan-out/Fan-in, Expert Pool, Supervisor, Hierarchical Delegation)은 비도입 사유 명시

- **신규 검증 도구 `validate.ps1`**
  - 설치 후 plugin 정상 등록 자동 검증
  - 8가지 검증 항목: 디렉터리, JSON 파싱, skill 6, agent 6, hook 5, BOM, 토글, AGENTS.md.template
  - PASS/FAIL/WARN 카운트 + 상세 보고

**비도입 항목** (revfactory/harness에서 검토 후 제외)
- Meta-skill 도입 (종철님 plugin은 단일 사용자·단일 도메인용)
- Fan-out/Fan-in 패턴 (sequential 자율 루프 안정성 우선)
- Expert Pool / Supervisor / Hierarchical Delegation (현재 규모에 불필요)

### 1.10.0 — Track 1+2 최적화 (중복 통합 + Type-aware)

**Track 1 — 명백한 중복 통합**
- `plan-feature` 절대 규칙 **11개 → 7개** 통합
  - "팩트 기반 / 전수 확인 / 예측 금지" 3개 → "팩트 기반 — 예측 금지, 전수 확인" 1개
  - "불확실하면 질문 / 결정 사전화 / 자율 실행 전제" 3개 → "결정 사전화 — 자율 실행 전제" 1개
  - 의미 중복 4-5개 제거, 핵심 메시지 유지
- `implement-task` 절대 규칙 **16개 → 10개** 통합
  - "환각 금지 / 직접 확인 / 예측 금지" 3개 → "검증된 코드만 사용" 1개
  - "DDD / YAGNI" 2개 → 1개
  - "파괴적 작업 / 새 의존성" 2개 → 1개
- `code-quality-reviewer` 항목 **I 제거** (Cross-File Consistency)
  - spec-compliance-reviewer 항목 G로 일원화
  - code-quality는 명확성 + enum/예외 정적 일관성만 검사 (항목 H로 흡수)

**Track 2 — Type-aware 적용**
- `plan-feature` Step 6 (Decision Points 11개) → Type별 적용
  - Type A: skip / B: 1-2개 / C: 5-6개 / D: 전체
- `plan-feature` Step 6.5-A (Edge Cases 10개) → Type별 적용
  - Type A: skip / B: 빈입력+경계값 / C: 5-6개 / D: 전체
- `plan-reviewer` 11개 항목 → Type별 적용
  - Type A: 4개 / Type B: 8개 / Type C: 10개 / Type D: 전체

**효과 (정직한 실측 / 추정 구분)**

| 영역 | 변화 | 종류 |
|---|---|---|
| 절대 규칙 명확성 | 11+15+ → 7+10 (의미 단위) | 실측 |
| 검증 항목 중복 | `code-quality I` 제거 (spec-compliance G로 일원화) | 실측 |
| SKILL.md 라인 수 | **약 +25줄 (감소 아님)** — Type-aware 표 추가가 통합 효과를 상쇄 | 실측 |
| Type A/B plan 검토 시간 | 항목 적용 범위 축소로 단축 예상 | 추정 (실측 안 됨) |
| 검증 결정성 | Type별 적용으로 큰 plan에서는 동일, 작은 plan에서는 약간 약화 | 추정 |

> **솔직히**: 라인 수는 줄지 않았고, 토큰 절감 효과도 측정 안 됨.
> 진짜 가치는 "**규칙의 의미가 명확해진 것**"과 "**작은 plan에서 적용 항목이 줄어든 것**".
> 큰 plan에서는 1.9.0과 거의 동일하게 동작.

### 1.9.0 — Trivial Bypass + Phase F 조건부 (간단 작업 가속)
- **문제**: 1줄 문구 변경, 아이콘 교체 같은 1분 작업이 plan + 검증으로 20분 소요
- **해결 1**: plan-feature description에 Trivial Bypass 명시
  - UI 문구·라벨, 아이콘/이미지 교체, 색상 토큰, 오타, 주석 추가 등은 plan 없이 직접 처리
  - 4가지 trivial 판정 기준 명시 (단일 파일·로직 변경 없음·영향 0·의도 명확)
  - 불확실하면 plan-feature 사용 (안전 우선)
- **해결 2**: require-plan-for-write hook에 trivial 파일 예외 확장
  - 이미지 확장자 허용: `.svg`, `.png`, `.jpg`, `.jpeg`, `.gif`, `.ico`, `.webp`, `.bmp`
  - 리소스 확장자 허용: `.resx`, `.resw`
  - 파일명 패턴 허용: `README`, `CHANGELOG`, `LICENSE`, `strings.xml`, `Localizable.strings` 등
  - Android 리소스 디렉터리 허용: `res/values/`, `res/drawable/`, `res/mipmap/` 등
- **해결 3**: Phase F 조건부 진입
  - task 1개 + Type A만 → Phase F 전체 생략
  - task 1개 + Type B → F-1, F-2, F-6만 (F-7 Opus subagent 생략)
  - task 2개 이상 또는 Type C/D 포함 → Phase F 전체 (기존 동작)
- **안전망**: trivial 작업도 PostToolUse impact-warn hook이 자동으로 caller 영향 검증

**효과 비교** (추정, 실제는 다를 수 있음):
| 시나리오 | 1.8.0 | 1.9.0 |
|---|---|---|
| 버튼 라벨 변경 | ~20분 | ~1분 |
| 아이콘 SVG 교체 | ~10분 | ~30초 |
| README 오타 수정 | ~5분 | ~10초 |
| 단일 메서드 추가 (Type C) | 동일 | 동일 |
| Cross-file 리팩토링 (Type D) | 동일 | 동일 |

### 1.8.0 — Caller 영향 자동 경고 + Type B V-7 의무화
- 신규 hook **`impact-warn.ps1`** (PostToolUse, Write|Edit)
  - 모든 Write/Edit 후 자동 실행
  - 변경 파일의 public/internal/export 심볼 검출
  - 그 심볼을 git grep으로 caller 검색
  - caller 파일 목록을 stderr 경고로 출력 (차단 X)
  - 다언어 지원: C#, TS/JS, Python, Java/Kotlin, Go, Rust
  - 토글 가능 (`harness-toggle impact-warn off`)
- Type B의 V-7 (Caller Re-verification) **의무화**
  - 1.7.0까지 Type B는 V-7 생략 → Type 분류 오류 시 caller 누락 가능
  - 이제 모든 Type B/C/D에서 V-7 실행
- implement-task V-4에 impact-warn 경고 대응 절차 명시
  - 경고 발생 시 caller 파일 자동 Read → 영향 검증 → 함께 수정
- harness-toggle 헬퍼에 impact-warn 등록
- README의 "문제 1 — 연관 파일 누락" 방어 계층 6단계로 확장

**적용 케이스**:
- 짧은 수정 요청("이 메서드 수정해줘") 시에도 caller 영향 자동 검출
- plan-feature를 우회한 quick 모드에서도 안전망 작동
- 토큰 비용 거의 0 (grep 1회 ~1초)

### 1.7.0 — /goal 좋은 점 차용 (cheap validator + execution stats)
- `/goal` 기능 분석 후 plugin에 부분 차용 (대체 아닌 보완)
- 신규 subagent **`spec-prefilter`** (Haiku)
  - Type B (Trivial Code) task의 V-5 빠른 1차 필터
  - PASS → Sonnet 호출 안 함 (토큰 절감)
  - ESCALATE → 기존 spec-compliance-reviewer (Sonnet) 호출
- implement-task 진행 보고 강화
  - 각 task 보고에 Type, Phase V 적용 단계, Elapsed, Turn 표시
  - 커밋 메시지에 Type/Prefilter 결과/Elapsed 추가
  - 최종 보고에 누적 stats (elapsed, turns, Type 분포, prefilter PASS율)
- 검토 결과: /goal로 plugin 대체는 비추 (사전 계획·다층 검증·한국어 가치 손상)
- 추가 차용 안 한 항목:
  - OS-level persistence (plan.md + git checkpoint로 충분)
  - 단일 진입점 명령 (plan-feature → implement-task 2단계 유지)

### 1.6.0 — Phase F (Finalize) + plan-completion-reviewer 추가
- 자율 루프 마지막 안전망 추가
- 모든 task 완료 → 곧장 보고하지 않고 **Phase F 통합 검증**
  - F-1 Goal 재확인
  - F-2 전체 빌드 + 전체 테스트 (per-task 아닌 전체)
  - F-3 Impact Analysis 회귀 점검
  - F-4 Follow-ups 완전성 확인
  - F-5 Risks & Unknowns 실현 검토
  - F-6 자기 정직성 최종 체크
  - F-7 **`plan-completion-reviewer` subagent 의무 호출**
- 신규 subagent `plan-completion-reviewer` (Opus)
  - 9개 검증 항목 (Goal/Acceptance/Impact/Cross-Task/회귀/Risks/Edge Cases/Follow-ups/자기기만)
  - V-5(spec-compliance)와 차이: task별이 아닌 **plan 전체** 적대적 검토
  - 빌드·테스트 직접 재실행
- 자율 루프 흐름도에 Phase F 명시
- README subagent 4개 → 5개 갱신

### 1.5.0 — 종합 개선 (7개 항목 보완)

**자율성 경계 명시**
- `plan-feature`: USER-INTERACTIVE 모드 명시 (질문 OK, 사용자 승인 1회)
- `implement-task`: FULLY AUTONOMOUS 모드 명시 (질문 금지, Halt만 가능)
- 두 skill 간 경계 다이어그램 추가

**Fast-Path 도입 (Phase V 단계 폭증 해결)**
- Task Type 분류 (A/B/C/D) 도입 — plan-feature Step 5에서 의무 명시
- Phase V Fast-Path: Type별로 다른 단계 적용
  - A (Doc/Config): V-1 + V-8
  - B (Trivial): V-1 + V-2 + V-5(빠른 모드) + V-8
  - C (Normal): V-1~V-3 + V-5 + V-7 + V-8
  - D (Complex): V-1~V-8 전체
- plan.md 템플릿에 `Type:` 필드 추가

**Sub-skill 호출 흐름 명시**
- `add-viewmodel` / `add-domain-service`: implement-task Phase I 안에서 호출됨을 명시
- 책임 범위 분리: sub-skill = boilerplate, implement-task = 비즈니스 로직 + 검증
- Android의 Jetpack ViewModel은 add-viewmodel 비대상임을 재확인

**plan-reviewer 13개 → 11개 통합**
- 구 1번 + 구 10번 → 1. Speculation & Hallucination Detection
- 구 3번 + 구 3-B + 구 9번 → 3. Impact Coverage (3-A/3-B/3-C 하위)
- 자율 실행 관련 11/12/13 → 9/10/11로 재번호
- 중복 제거로 동일 이슈가 여러 항목으로 보고되던 문제 해결

**plan.md 경로 단일화**
- AGENTS.md.template에 `Plan Location:` 설정 추가
- plan-feature Step 7이 AGENTS.md 설정 우선 적용

**harness-toggle 트리거 구체화**
- description에 "harness/hook 컨텍스트 단어 필수" 명시
- "이 기능 끄는 코드" 같은 일반 표현으로 잘못 트리거되는 문제 차단

### 1.4.4 — 자율 실행을 위한 계획 단계 보강
- 1.4.3에서 자율 루프를 강제했으므로, plan 단계가 더 철저해야 함
- plan-feature 절대 규칙 #9 추가: "자율 실행 전제"
  - "이 plan을 다른 사람에게 넘겨도 추가 질문 없이 끝낼 수 있는가?" 자기 검증
- 신규 Step 6.5 — Edge Case & Halt Forecast
  - 6.5-A: Edge Case 커버리지 (빈 입력/경계/동시성/권한/네트워크 등 10개 카테고리)
  - 6.5-B: Halt Forecast — 구현 중 멈춤 시나리오 사전 예측 + plan 내 해결책 지목
  - 6.5-C: 자율 실행 준비도 자문 3개 질문
- plan.md 템플릿에 `Edge Cases`, `Halt Forecast` 섹션 추가
- 통과 체크리스트에 4개 항목 추가
- plan-reviewer에 신규 검증 3개 추가:
  - 11. Autonomous Readiness (자율 실행 준비도)
  - 12. Edge Case 커버리지
  - 13. Halt Forecast 부재

### 1.4.3 — 자율 루프 강제력 보강
- 증상: 10개 task 계획해도 1개 진행 후 "다음 진행할까요?" 묻고 멈춤
- 원인: implement-task의 자율성 표현이 약함, "확인 요청" 안티패턴 미명시
- 수정:
  - description에 "FULLY AUTONOMOUS loop, never asks between tasks" 명시
  - 자율 루프 절대 규칙 5가지 추가 (task 사이 사용자 확인 금지 등)
  - 진행 보고 양식: "✅ T1 완료 (1/10) → T2 시작" — 알림이지 확인 요청 아님
  - 금지 표현 5개 명시: "T2 진행할까요?" 등
  - 안티패턴 표에 자율 루프 관련 4개 추가
  - 사용자 개입 지점은 단 2개로 못박음 (plan 승인 + Halt/완료)
  - README의 implement-task 섹션에 자율 루프 원칙 표 추가

### 1.4.2 — AGENTS.md.template 멀티스택 지원
- WinUI 3 / .NET 예시 + Android / Kotlin 예시를 함께 제공
- 사용자가 자기 스택 부분만 남기고 나머지는 삭제하는 방식
- Android 케이스에서 `pjc:add-viewmodel`은 WinUI 3 전용임을 명시

### 1.4.1 — 인코딩 버그 수정
- 모든 `.ps1` 파일에 UTF-8 BOM 추가
  - 원인: PowerShell 5.1이 BOM 없는 UTF-8 파일을 시스템 기본 인코딩(CP949)으로 잘못 해석
  - 증상: `install.ps1 -Uninstall` 실행 시 한글이 깨지면서 `MissingEndCurlyBrace` 파싱 오류 발생
- `check-utf8-and-lines.ps1` BOM 검사에서 `.ps1` 파일 제외
  (`.ps1`은 BOM 필수, 일반 코드는 BOM 없음이 표준)

### 1.4.0
- `implement-task` Phase V-7 (Caller Re-verification) 추가
- `implement-task` Phase V-8 (Self-Honesty Check) 추가
- `plan-reviewer` 3-B (Breaking Change Propagation) 검출 추가
- `spec-compliance-reviewer` G (Cross-File Caller Impact) 추가
- `spec-compliance-reviewer` H (Evidence Honesty) 추가
- 자기기만·누락 차단 다층 방어 메커니즘 README 문서화

### 1.3.0
- `harness-toggle` skill 신규 (런타임 hook on/off)
- `harness-toggle.ps1` 헬퍼 스크립트
- 각 hook에 토글 분기 추가

### 1.2.0
- `install.ps1` 자동 설치 스크립트 추가 (-Scope, -Uninstall, -SkipVerification)
- 상세 README (Skill 6개 / Subagent 4개 / Hook 4개 / 첫 사용 walkthrough)

### 1.1.0
- 초기 plugin 패키지 (skills 5개, agents 4개, hooks 4개)
- `pjc:` namespace 자동 prefix
- `${CLAUDE_PLUGIN_ROOT}` 사용

### 1.0.0
- Standalone 글로벌 버전 (PowerShell)
